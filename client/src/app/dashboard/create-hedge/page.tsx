...
function CreateHedgeForm() {
  ...
  } = useFaucetStatus(address as `0x${string}`);
  ...
  useEffect(() => {
    if (isFaucetSuccess) {
      refetchFaucetStatus().then(() => {
        toast.success("Mock USDT Claimed! 💰", { 
          description: "Your on-chain claim was successful. You now have test tokens to start hedging." 
        });
      });
    }
  }, [isFaucetSuccess, refetchFaucetStatus]);
  ...
  const handleClaimFaucet = () => {
    ...
    claimFaucet({
      ...
      abi: mockUsdtAbi,
      functionName: "faucet",
      gas: BigInt(150000), // Slightly higher gas buffer for Injective inEVM
    });
  };
  ...
  if (isFaucetStatusLoading) {
    return (
      <div className="w-full h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">Syncing On-Chain Status...</p>
      </div>
    );
  }
